...
class TcpServer:
    def __init__(self, host_ip=tcp_config.HOST_IP, port=tcp_config.PORT, frame_ref=None, frame_lock=None, camera_matrix=None, dist_coeffs=None, aruco_z=None):
        self.host = host_ip
        self.port = port
            ...
            while True:  # 클라이언트와 연결을 끊지 않고 계속 처리
                ...
                with self.frame_lock:
                    if self.frame_ref[0] is not None:
                        ...
                        print(f"아루코 z값: {self.aruco_z}")

                        if self.aruco_z:
                            response = {
                                "success": True,
                                "book_title": data,
                                "aruco_z": self.aruco_z
                            }
                        else:
                            response = {
                                "success": False,
