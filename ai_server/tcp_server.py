...
def run_yolo(img_path):
    model = YOLO("/home/addinedu/Downloads/yolo11s_best.pt")
    # yolo 처리 이후에 이미지
    after_yolo_img_list = []
    # 책 위치 정보 리스트
    book_location_list = []
...
def run_ocr(image_list):
    ...
    ocr = PaddleOCR(lang='korean', use_gpu=True, show_log=False)
    search_keyword_list = []
    ...
    for img in image_list:
        ...
        # font = ImageFont.truetype("./MaruBuri-Bold.ttf", 20)
...
def communication_tcp(book_data):
    HOST = '192.168.0.153' ## server에 출력되는 ip를 입력해주세요 ##
    PORT = 9999
...
def main1(title):
    img_path = "/home/addinedu/Downloads/output.png"
    ...
    for i in range(0, len(book_results_list)):
        print(book_results_list[i])
        print(book_results_list[i]['title'])
        print(title)
        if book_results_list[i]['title'] == title:
            print(book_location_list[i])
            return
